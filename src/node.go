...
func main() {
	...
	go func(thisElevatorHeartbeatCh chan<- msgs.Heartbeat, elevatorsStatusCh <-chan []msgs.ElevatorStatus, downedElevatorsCh <-chan []msgs.Heartbeat,
		thisTakeOrderChan <-chan msgs.TakeOrderMsg, otherTakeOrderCh chan<- msgs.TakeOrderMsg, completedOrderCh chan<- msgs.Order) {
		...
		go bcast.Receiver(port, placeOrderCh, acceptOrderCh)

		const fsmMaxFloor int = 10
		fsmStatus := msgs.ElevatorStatus{ID: *id_ptr, Direction: msgs.Up, Floor: rnd.Intn(fsmMaxFloor)}

		fmt.Println("Start fsm: ", fsmStatus)
		...
		for {
			...
			case placeOrder := <-placeOrderCh:
				if placeOrder.RecieverID == *id_ptr {
					order := msgs.OrderPlacedMsg{SenderID: *id_ptr, Order: placeOrder.Order}
			...
			case acceptOrder := <-acceptOrderCh:
				if acceptOrder.RecieverID == *id_ptr {
					if _, exists := orders[acceptOrder.OrderID]; exists {
						acceptedOrders[acceptOrder.OrderID] = true

						scoreMap := make(map[string]int)
						for _, elevator := range elevators {
							scoreMap[elevator.ID] = calculateOrderScore(elevator, orders[acceptOrder.OrderID])
						}
						...
						fmt.Printf("[orderHandler]: elevator %v should take order %v (%v)\n", bestID, acceptOrder.OrderID, scoreMap)
						if bestID != *id_ptr {
							takeOrderMsg := msgs.TakeOrderMsg{SenderID: *id_ptr, CmdID: bestID, Order: orders[acceptOrder.OrderID]}
							otherTakeOrderCh <- takeOrderMsg
						} else {
							thisElevatorOrders[acceptOrder.OrderID] = true
						}
					} else {
			...
			case msg := <-thisTakeOrderCh:
				if _, exists := orders[msg.Order.ID]; !exists {
					fmt.Printf("[thisTakeOrderCh]: didnt have order %v\n from before", msg.Order.ID)
	...
	}(thisElevatorHeartbeatCh, elevatorsStatusCh, downedElevatorsCh, thisTakeOrderCh, otherTakeOrderCh, completedOrderCh)
	...
	for {
		...
		case msg := <-takeOrderRecvCh:
			if msg.CmdID == *id_ptr {
				fmt.Printf("TODO: Need to take order %v and send ack\n", msg.Order)
			}
		...
		for orderID, t := range takeUnackedOrders {
			if time.Now().Sub(t) > giveupAckwaitTimeout {
				...
				msg := msgs.TakeOrderMsg{SenderID: *id_ptr, CmdID: *id_ptr,
