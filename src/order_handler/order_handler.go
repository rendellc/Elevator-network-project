...
func OrderHandler(simAddr string, addHallOrderCh chan<- OrderEvent, deleteHallOrderCh chan<- OrderEvent,
	placedHallOrderCh <-chan OrderEvent, completedOrderThisElevCh <-chan []OrderEvent,
	elevatorStatusCh <-chan Elevator, turnOnLightsCh chan<- [N_FLOORS][N_BUTTONS]bool,
    thisElevatorHeartbeatCh chan<- msgs.Heartbeat, completedOrderOtherElevCh chan<- msgs.Orders,
    ...
    placedOrderCh, completedOrderCh, thisElevatorHeartbeatCh
...
func simplified_order_controller(addHallOrderCh chan<- fsm.OrderEvent, deleteHallOrderCh chan<-fsm.OrderEvent,
  placedHallOrderCh <-chan fsm.OrderEvent, completedOrderThisElevCh <-chan []fsm.OrderEvent,
  ...
  ch := make(chan string)
    ...
    case  <- completedOrderThisElevCh:
      //fmt.Println("Completed placedOrders: ", a)
...
func main(){
  ...
  go fsm.FSM("localhost:15657",addHallOrderCh, deleteHallOrderCh, placedHallOrderCh, completedOrderThisElevCh, elevatorStatusCh)
  go simplified_order_controller(addHallOrderCh, deleteHallOrderCh, placedHallOrderCh, completedOrderThisElevCh, elevatorStatusCh)
